public class MessageCensor {
	...
	public static final String apply(String message) {
	...
	public static final int hash(char[] chars) {
		if (chars.length > 6) {
			return 0;
		}
	...
	public static final void init(Archive archive) {
		Buffer fragments = new Buffer(archive.extract("fragmentsenc.txt"));
		Buffer bad = new Buffer(archive.extract("badenc.txt"));
		Buffer domain = new Buffer(archive.extract("domainenc.txt"));
		Buffer tldlist = new Buffer(archive.extract("tldlist.txt"));
		decode(fragments, bad, domain, tldlist);
	}

	public static final void method509(byte[][] complex, char[] text, char[] textToCensor) {
		if (textToCensor.length > text.length) {
			return;
		}

		int j;
		for (int i = 0; i <= text.length - textToCensor.length; i += j) {
			int censorEnd = i;
			...
			while (censorEnd < text.length && (!nextCharacterIsDigit || !finalCharacterIsDigit)) {
				int k1 = 0;
				char charAtEnd = text[censorEnd];
				char nextCharacter = '\0';
				if (censorEnd + 1 < text.length) {
					nextCharacter = text[censorEnd + 1];
				}
				if (characterCount < textToCensor.length
						&& (k1 = method512(nextCharacter, charAtEnd, textToCensor[characterCount])) > 0) {
				...
				if ((k1 = method512(nextCharacter, charAtEnd, textToCensor[characterCount - 1])) > 0) {
					censorEnd += k1;
					if (characterCount == 1) {
						j++;
					}
				...
				if (characterCount >= textToCensor.length || !method518(charAtEnd)) {
					break;
				}
			...
			if (characterCount >= textToCensor.length && (!nextCharacterIsDigit || !finalCharacterIsDigit)) {
				boolean applyCensor = true;
				if (!finalCharacterIsNotAlphanumeric) {
					char characterBeforeBeginning = ' ';
					if (i - 1 >= 0) {
						characterBeforeBeginning = text[i - 1];
					}
					char characterAtEnd = ' ';
					if (censorEnd < text.length) {
						characterAtEnd = text[censorEnd];
					}
					...
					if (i - 1 < 0 || isNotAlphanumeric(text[i - 1]) && text[i - 1] != '\'') {
					...
					if (!initalCharacterIsInvalid || !finalCharacterIsInvalid) {
						...
						for (; !valid && position < censorEnd; position++) {
							if (position >= 0 && (!isNotAlphanumeric(text[position]) || text[position] == '\'')) {
								char chars[] = new char[3];
								int idx;
								for (idx = 0; idx < 3; idx++) {
									if (position + idx >= text.length || isNotAlphanumeric(text[position + idx])
											&& text[position + idx] != '\'') {
										break;
									}
									chars[idx] = text[position + idx];
								}
								...
								if (idx < 3 && position - 1 >= 0
										&& (!isNotAlphanumeric(text[position - 1]) || text[position - 1] == '\'')) {
									ok = false;
								}
	...
	private static final void capitalise(char[] chars) {
		boolean flag = true;
	...
	private static final void censorDomains(char[] chars) {
	...
	private static final boolean containsFragment(char[] chars) {
	...
	private static final void copyCase(char[] originalText, char[] censoredText) {
	...
	private static final void decode(Buffer fragments, Buffer badenc, Buffer domainenc, Buffer tldlist) {
		decodeBad(badenc);
	...
	private static final void decodeBad(Buffer buffer) {
	...
	private static final void decodeDomain(Buffer buffer) {
		int length = buffer.readInt();
		domains = new char[length][];
		initDomains(domains, buffer);
	}

	private static final void decodeFragments(Buffer buffer) {
	...
	private static final void decodeTlds(Buffer buffer) {
		int length = buffer.readInt();
	...
	private static final byte getCensoringCharacterCode(char character) {
		if (character >= 'a' && character <= 'z') {
	...
	private static final int indexOfDigit(char chars[], int offset) {
	...
	private static final int indexOfNonDigit(char chars[], int offset) {
	...
	private static final void initBad(Buffer buffer, char[][] words, byte[][][] complexBadEnc) {
		for (int index = 0; index < words.length; index++) {
			char[] word = new char[buffer.readUByte()];
			for (int j = 0; j < word.length; j++) {
				word[j] = (char) buffer.readUByte();
			}
	...
	private static final void initDomains(char[][] domains, Buffer buffer) {
	...
	private static final boolean isNotAlphanumeric(char c) {
	...
	private static final boolean legalCharacter(char c) {
		return c >= ' ' && c <= '\177' || c == ' ' || c == '\n' || c == '\t' || c == '\243' || c == '\u20AC';
	}

	private static final void method500(char ac[]) {
		for (int i = 0; i < 2; i++) {
			for (int j = badWords.length - 1; j >= 0; j--) {
				method509(badEncoding[j], ac, badWords[j]);
			}
		}
	}

	private static final void method501(char ac[]) {
		char[] clone = ac.clone();
		char[] at = { '(', 'a', ')' };
	...
	private static final void method502(char ac[], char ac1[], char ac2[], char ac3[]) {
	...
	private static final int method503(char ac[], char ac1[], int j) {
		if (j == 0) {
			return 2;
		}
	...
	private static final int method504(char ac[], int i, char ac1[]) {
		if (i + 1 == ac1.length) {
			return 2;
		}
	...
	private static final void method506(char ac[], char ac1[], int i, char ac2[], char ac3[]) {
	...
	private static final int method507(char ac[], int j, char ac1[]) {
		if (j == 0) {
			return 2;
		}
		for (int k = j - 1; k >= 0; k--) {
	...
	private static final int method508(char ac[], char ac1[], int i) {
		if (i + 1 == ac.length) {
			return 2;
		}
	...
	private static final boolean method510(byte byte0, byte abyte0[][], byte byte2) {
		int i = 0;
	...
	private static final int method511(char c, char c1, char c2) {
		if (c1 == c) {
			return 1;
		}
		if (c1 == 'o' && c == '0') {
	...
	private static final int method512(char c, char c1, char c2) {
		if (c2 == c1) {
			return 1;
		}
		if (c2 >= 'a' && c2 <= 'm') {
	...
	private static final void method514(char chars[]) {
		int j = 0;
		int offset = 0;
		int l = 0;
		int i1 = 0;
	...
	private static final boolean method518(char c) {
		if (c < 'a' || c > 'z') {
			return true;
		}
	...
	private static final void stripIllegalCharacters(char chars[]) {
		int index = 0;
