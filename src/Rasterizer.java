public class Rasterizer extends Raster {
	...
	public static IndexedImage[] floorImages = new IndexedImage[50];
	...
	static int[][] floorPalettes = new int[50][]; // [texture][floor_raster[i]]
	static int loadedFloorCount;
	...
	public static final void loadFloorImages(Archive archive) {
		loadedFloorCount = 0;
		for (int id = 0; id < 50; id++) {
			try {
				floorImages[id] = new IndexedImage(archive, String.valueOf(id), 0);

				if (lowMemory && floorImages[id].resizeWidth == 128) {
					floorImages[id].downscale();
				} else {
					floorImages[id].resize();
				}
	...
	public static final void method365(int j, int k) {
	...
	public static final int method369(int texture) {
		...
		int g = 0;
		int b = 0;
		int count = floorPalettes[texture].length;
		for (int i = 0; i < count; i++) {
			r += floorPalettes[texture][i] >> 16 & 0xff;
			g += floorPalettes[texture][i] >> 8 & 0xff;
			b += floorPalettes[texture][i] & 0xff;
		}
	...
	public static final void method370(int i) {
	...
	public static final int[] method371(int id) {
		...
		if (anInt1477 > 0) {
			...
			int j = 0;
			int k = -1;
			for (int l = 0; l < loadedFloorCount; l++) {
		...
		IndexedImage image = floorImages[id];
		int ai1[] = floorPalettes[id];
		if (lowMemory) {
			aBooleanArray1475[id] = false;
			for (int i1 = 0; i1 < 4096; i1++) {
				int i2 = ai[i1] = ai1[image.raster[i1]] & 0xf8f8ff;
				if (i2 == 0) {
			...
			if (image.width == 64) {
				for (int j1 = 0; j1 < 128; j1++) {
					for (int j2 = 0; j2 < 128; j2++) {
						ai[j2 + (j1 << 7)] = ai1[image.raster[(j2 >> 1) + (j1 >> 1 << 6)]];
					}
				}
			} else {
				for (int k1 = 0; k1 < 16384; k1++) {
					ai[k1] = ai1[image.raster[k1]];
				}
	...
	public static int method373(int colour, double d) {
	...
	public static final void method374(int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2) {
		int j2 = 0;
		int k2 = 0;
		if (j != i) {
			j2 = (i1 - l << 16) / (j - i);
	...
	public static final void method378(int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2,
	...
	public static final void method379(int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
	...
	public static final void useViewport() {
